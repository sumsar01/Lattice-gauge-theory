...
while e < 3:
    ...
    loop_expt2 = expect(Ex_ope(x, y, i, j),state)
    loop_expt3 = expect(Ey_ope(x, y, i, j),state)
...
plt.title('Electric field configuration on 2x2 lattice')
plt.xlabel('Charge (q)')
plt.ylabel('E-field')
plt.axis([np.sqrt(alpha), 3, -0.2, 5])
plt.savefig('E-field_config.svg')  
plt.show()
